...
namespace HighPingKicker
{
    public class HighPingKicker : Fougerite.Module
    {
        ...
        public override string Description { get { return "Kicks high ping players"; } }
        ...
        private void LoadConfig()
        {
            if (!File.Exists(Path.Combine(ModuleFolder, "Config.ini")))
            {
                ...
                config.Save();
                Logger.Log("HighPingKicker Plugin: New configuration file generated");
                LoadConfig();
            }
            else
            {
                ...
                try
                {
                    ...
                    Logger.Log("HighPingKicker Plugin: Configuration file loaded");
                }
                catch (Exception ex)
                {
                    Logger.LogError("HighPingKicker Plugin: Detected a problem in the configuration");
                    Logger.Log("ERROR -->" + ex.Message);
                    File.Delete(Path.Combine(ModuleFolder, "Config.ini"));
                    Logger.LogError("HighPingKicker Plugin: Deleted the old configuration file");
                    LoadConfig();
                }
            }
        }
